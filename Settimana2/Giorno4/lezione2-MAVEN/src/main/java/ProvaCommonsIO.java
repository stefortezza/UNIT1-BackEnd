...
public class ProvaCommonsIO {
    public static void main(String[] args) {
        ...
        try {
            FileUtils.writeStringToFile(file, "CIAO\n", Charset.defaultCharset(), true); //LA SCRITTURA VA IN SOSTITUZIONE
        ...
        List<String> prodottiFormattati = prodotti.stream()
                .map(prodotto ->
                        prodotto.getId().longValue() + "@" +
                                prodotto.getNome() + "@" +
                                prodotto.getCategoria() + "@" +
                                prodotto.getPrezzo())
                .collect(Collectors.toList());

        prodottiFormattati.forEach(System.out::println);
    }
}
